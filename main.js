...
bot.on("message", async (msg) => {
  try {
    ...
    if (isBanned(userId)) {
      return bot.sendMessage(chatId, "🚫 | 𝐘𝐨𝐮 𝐡𝐚𝐯𝐞 𝐛𝐞𝐞𝐧 𝐛𝐚𝐧𝐧𝐞𝐝 𝐟𝐫𝐨𝐦 𝐮𝐬𝐢𝐧𝐠 𝐭𝐡𝐞 𝐛𝐨𝐭");
    }
  ...
  return; 
    ...
    if (command.config?.usePrefix && !text.startsWith(prefix)) return;
    ...
    if (cooldownTime > 0) {
      ...
      if (timestamps.has(userId) && now < timestamps.get(userId)) {
        const left = ((timestamps.get(userId) - now) / 1000).toFixed(1);
        return message.reply(`⏳ | 𝐏𝐥𝐞𝐚𝐬𝐞 𝐰𝐚𝐢𝐭 ${left}s`);
      }
    ...
    if (role === 2 && !isBotAdmin)
      return message.reply("👽🔖 | 𝐎𝐧𝐥𝐲 𝐛𝐨𝐭 𝐚𝐝𝐦𝐢𝐧 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐭𝐡𝐢𝐬");

    if (role === 1 && !isBotAdmin && !isAdmin)
      return message.reply("👽🔖 | 𝐎𝐧𝐥𝐲 𝐠𝐫𝐨𝐮𝐩 𝐚𝐝𝐦𝐢𝐧 𝐜𝐚𝐧 𝐮𝐬𝐞");

    if (role === 3 && !isBotAdmin && !isOperator)
      return message.reply("👽🔖 | 𝐎𝐧𝐥𝐲 𝐨𝐩𝐞𝐫𝐚𝐭𝐨𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞");
